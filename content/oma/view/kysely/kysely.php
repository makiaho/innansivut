...
class SomeViewKysely extends SomeView {
  ...
  //->
  public function display($tmpl = 'kysely') {
  
  
	//mallin saa getModel() metodilla. 
	//  Se on malli, jonka controlleri t�lle oliolle asetti kutsumalla $view->setModel($model)
	$model = $this->getModel();
	//mallissa voi olla meille dataa t�ss� tapauksessa haettavana metodilla getNow()
	// $this->showdate on k�yt�ss� template tiedostossa tmpl/default.php, sill� t�m� olio $this hakee tuon template tiedoston
	//  parent luokan loadTemplate metodissa.
	//$this->showdate = $model->getNow();
