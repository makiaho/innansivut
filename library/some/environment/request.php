<?php
/**
* ready made safe implementation on http request sanitation
* @package wo2009
* @subpackage library
*/
...
class SomeRequest
{
	...
	public static function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
	{
		...
		if (!isset($GLOBALS['_JREQUEST'][$name][$sig]))
		{
			if (isset($input[$name]) && $input[$name] !== null) {
				// Get the variable from the input hash and clean it
				$var = SomeRequest::_cleanVar($input[$name], $mask, $type);

				// Handle magic quotes compatability
				if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
					$var = SomeRequest::_stripSlashesRecursive( $var );
				}
			...
			elseif ($default !== null) {
				...
				$var = SomeRequest::_cleanVar($default, $mask, $type);
			}
			else {
				$var = $default;
			}
	...
	{
		return SomeRequest::getVar($name, $default, $hash, 'int');
	}
	...
	{
		return SomeRequest::getVar($name, $default, $hash, 'float');
	}
	...
	{
		return SomeRequest::getVar($name, $default, $hash, 'bool');
	}
	...
	{
		return SomeRequest::getVar($name, $default, $hash, 'word');
	}
	...
	{
		return SomeRequest::getVar($name, $default, $hash, 'cmd');
	}
	...
	public static function getString($name, $default = '', $hash = 'default', $mask = 0)
	{
		...
		return (string) SomeRequest::getVar($name, $default, $hash, 'string', $mask);
	}
	...
	public static function get($hash = 'default', $mask = 0)
	{
		...
		$result = SomeRequest::_cleanVar($input, $mask);

		// Handle magic quotes compatability
		if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
			$result = SomeRequest::_stripSlashesRecursive( $result );
		}
	...
	public static function checkToken( $method = 'post' )
	{
		...
		if(!SomeRequest::getVar( $token, '', $method, 'alnum' )) {
...
;				$mainframe->redirect($return, JText::_('SESSION_EXPIRED'));
	...
	{
		$value = is_array( $value ) ? array_map( array( 'SomeRequest', '_stripSlashesRecursive' ), $value ) : stripslashes( $value );
		return $value;
	}
}
